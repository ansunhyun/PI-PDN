...
HEADER_ALIASES: dict[str, set[str]] = {
    ...
    "syz_option": {"syzoption"},
    "tdr_option": {"tdroption"},
...
class DetailedSnpBatch:
    ...
    @property
    def syz_option(self) -> str | None:
        if not self.pairs:
            return None
        return self.pairs[0].syz_option or None

    def tdr_options_by_item(self) -> dict[str, str | None]:
        options: dict[str, str | None] = {}
        for pair in self.pairs:
            options.setdefault(pair.tdr_chart_name, pair.tdr_option or None)
        return options
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "syzOption": self.syz_option,
            "tdrOptionsByItem": self.tdr_options_by_item(),
        ...
        for line_number, raw_row in enumerate(reader, start=1):
            ...
            rows.append(
                DetailedConductorRow(
                    ...
                    syz_option=input_values.get("syz_option", ""),
                    tdr_option=input_values.get("tdr_option", ""),
...
def normalize_detailed_input(csv_input: DetailedCsvInput) -> DetailedNormalizationResult:
    ...
    for offset in range(0, len(csv_input.rows), 2):
        ...
        batch_hint, tdr_item_hint, _, _, _ = _resolve_output_grouping(first)
        batch_hint = batch_hint or first.snp_file or "<unresolved>"
        tdr_item_hint = tdr_item_hint or first.tdr_chart_name or first.group or "<unresolved>"
        if first.syz_option != second.syz_option:
            raise DetailedInputValidationError(
                f"sNp batch {batch_hint!r} paired rows have conflicting SYZ_Option "
                f"values: {[first.syz_option or '<administrator-default>', second.syz_option or '<administrator-default>']}"
            )
        if first.tdr_option != second.tdr_option:
            raise DetailedInputValidationError(
                f"sNp batch {batch_hint!r}, TDR item {tdr_item_hint!r} paired rows "
                f"have conflicting TDR_Option values: "
                f"{[first.tdr_option or '<administrator-default>', second.tdr_option or '<administrator-default>']}"
            )
        ...
        pair = DetailedChannelPair(
            ...
            syz_option=positive.syz_option,
            tdr_option=positive.tdr_option,
            channel=channel,
    ...
    profile_pairs_by_batch: dict[str, list[DetailedChannelPair]] = {}
    for _, pair, _ in accepted_candidates:
        profile_pairs_by_batch.setdefault(pair.snp_file.casefold(), []).append(pair)
    for batch_pairs in profile_pairs_by_batch.values():
        batch_id = batch_pairs[0].snp_file
        syz_values = sorted({pair.syz_option for pair in batch_pairs})
        if len(syz_values) > 1:
            display_values = [value or "<administrator-default>" for value in syz_values]
            raise DetailedInputValidationError(
                f"sNp batch {batch_id!r} has conflicting SYZ_Option values: "
                f"{display_values}"
            )
        tdr_pairs_by_item: dict[str, list[DetailedChannelPair]] = {}
        for pair in batch_pairs:
            tdr_pairs_by_item.setdefault(pair.tdr_chart_name.casefold(), []).append(pair)
        for item_pairs in tdr_pairs_by_item.values():
            item_id = item_pairs[0].tdr_chart_name
            tdr_values = sorted({pair.tdr_option for pair in item_pairs})
            if len(tdr_values) > 1:
                display_values = [
                    value or "<administrator-default>" for value in tdr_values
                ]
                raise DetailedInputValidationError(
                    f"sNp batch {batch_id!r}, TDR item {item_id!r} has conflicting "
                    f"TDR_Option values: {display_values}"
                )
