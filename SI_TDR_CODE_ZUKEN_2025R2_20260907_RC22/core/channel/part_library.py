...
an explicit regular expression. A regular-expression rule may declare
``sourceColumns`` so one administrator Config can select the corresponding
header from either a BOM or PartList export. The selected source header must be
unambiguous. The group then contributes optional ``siTdr`` Array facts.
``seriesTreatment.byGroup`` remains a separate Channel Path policy.
...
def _resolve_bom_path(
    ...
    *,
    config_path: Path,
    bom_path_override: Path | None = None,
) -> Path:
    if bom_path_override is not None:
        resolved_override = bom_path_override.resolve()
        if not resolved_override.is_file():
            raise PartLibrarySchemaError(
                f"{config_path}: external Job BOM file not found: {resolved_override}"
            )
...
def _resolve_requested_column(
    ...
    *,
    ...
    """Resolve configured BOM vocabulary through one unambiguous alias.

    Customer Config keeps canonical names such as ``Designator`` and
    ``Part Number``.  LG PartList exports use ``Reference Designator`` and
    ``Part No.``.  Exact headers win; alias fallback is accepted only when the
    source contains one member of that semantic family.
    """

    normalized_requested = _normalized_column(requested)
    exact = columns.get(normalized_requested)
    if exact is not None:
        return exact
    family = next(
        (
            aliases
            for aliases in _COLUMN_ALIAS_FAMILIES
            if normalized_requested
            in {_normalized_column(alias) for alias in aliases}
        ),
        None,
    )
    if family is None:
        return None
    normalized_family = {_normalized_column(alias) for alias in family}
    matches = sorted(
        {
            actual
            for normalized, actual in columns.items()
            if normalized in normalized_family
        },
        key=str.casefold,
    )
    if len(matches) > 1:
        raise PartLibrarySchemaError(
            f"{where} is ambiguous for configured column {requested!r}: {matches}"
        )
...
def _matches_filter(cell_value: str, rule: object, *, where: str) -> bool:
    ...
    if not isinstance(rule, dict) or set(rule) not in (
        ...
        raise PartLibrarySchemaError(
            f"{where} must be a string, string list, {{\"regex\": \"...\"}}, "
            "or a regex rule with sourceColumns"
        )
...
def _resolve_filter_column(
    ...
    *,
    ...
    """Resolve one physical CSV header for a configured group filter.

    Without ``sourceColumns`` the historical requested-column/alias behavior is
    preserved. With ``sourceColumns`` only those explicit candidates are
    considered; zero or multiple physical matches are rejected before row
    matching.
    """

    if not isinstance(rule, dict) or "sourceColumns" not in rule:
        return _resolve_requested_column(columns, requested, where=where)

    source_columns = rule.get("sourceColumns")
    if not isinstance(source_columns, list) or not source_columns:
        raise PartLibrarySchemaError(
            f"{where}.sourceColumns must be a non-empty string list"
        )
    candidates: list[str] = []
    for index, candidate in enumerate(source_columns):
        if not isinstance(candidate, str) or not candidate.strip():
            raise PartLibrarySchemaError(
                f"{where}.sourceColumns[{index}] must be a non-empty string"
            )
        candidates.append(candidate.strip())

    normalized_candidates = {_normalized_column(candidate) for candidate in candidates}
    matches = sorted(
        {
            actual
            for normalized, actual in columns.items()
    ...
    if len(matches) > 1:
        raise PartLibrarySchemaError(
            f"{where}.sourceColumns is ambiguous in the source file: "
...
def _normalize_bom_comp_prop_payload(
    ...
    *,
    ...
    config_path: Path,
    bom_path_override: Path | None = None,
    allow_unmatched_groups: bool = False,
) -> list[dict[str, Any]]:
    ...
    bom_path = _resolve_bom_path(
        payload,
        bom_config,
        config_path=config_path,
        bom_path_override=bom_path_override,
    )
    try:
    ...
    for required in col_key:
        if _resolve_requested_column(
            columns,
            required,
            where=f"{source}.BOM.colKey",
        ) is None:
            raise PartLibrarySchemaError(
    ...
    rows_by_designator: dict[str, list[dict[str, str]]] = {}
    for row in rows:
        for designator in _split_designator_cell(row.get(designator_column, "")):
            rows_by_designator.setdefault(_normalized_key(designator), []).append(row)
    ...
    for group_name, raw_group in comp_prop.items():
        ...
        for requested_column, rule in filters.items():
            column = _resolve_filter_column(
                ...
                requested_column,
                rule,
                where=f"{where} filter column",
            )
            if column is None:
                candidates = (
                    rule.get("sourceColumns")
                    if isinstance(rule, dict) and "sourceColumns" in rule
                    else [requested_column]
                )
                raise PartLibrarySchemaError(
                    f"{where} filter column {requested_column!r} is missing from {bom_path}; "
                    f"sourceColumns={candidates}"
                )
            ...
            entries.append(
                {
                    ...
                    **({"model": facts["model"]} if facts.get("model") else {}),
                }
            )
            continue
        ...
        # LG PartList may contain alternate parts for unrelated RefDes values.
        # Those rows must not block an SI-TDR group.  A RefDes selected by this
        # group must still have one identity across the complete PartList;
        # otherwise this group's pin map could target an unknown alternative.
        for designator in designators:
            source_rows = rows_by_designator.get(_normalized_key(designator), [])
            fingerprints = {
                tuple(
            ...
            if len(fingerprints) > 1:
                raise PartLibrarySchemaError(
                    f"{where}: matched BOM Designator {designator!r} has "
                    "conflicting identity rows"
                )
...
def normalize_part_library_payload(
    ...
    *,
    ...
    bom_path_override: Path | None = None,
    allow_unmatched_bom_groups: bool = False,
) -> list[dict[str, Any]]:
    ...
    if has_bom_rules:
        ...
        raw_entries.extend(
            _normalize_bom_comp_prop_payload(
                ...
                bom_path_override=bom_path_override,
                allow_unmatched_groups=allow_unmatched_bom_groups,
            )
        )
