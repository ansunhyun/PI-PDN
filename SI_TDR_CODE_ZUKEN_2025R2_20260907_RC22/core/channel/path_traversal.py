...
from .array_resistor_bom import is_array_refdes
...
def _array_mapping_candidate(
    ...
    pins_by_name = _component_pins_by_name(edb, refdes)
    try:
        resolved_pairs = mapping.resolve_pin_pairs(pins_by_name, refdes=refdes)
    except ValueError:
        return None
    ...
    for pin_name, pin in pins_by_name.items():
        if _pin_net(pin) != current_net:
            continue
        paired_pin_name = ArrayPartMapping(
            part_name=mapping.part_name,
            pin_pairs=resolved_pairs,
        ).paired_pin(pin_name)
    ...
    *,
    ...
    while queue:
        ...
        for name, component in sorted(components.items(), key=lambda item: item[0]):
            ...
            if is_array_refdes(name):
                array_candidate = _array_mapping_candidate(
                    edb, name, component, current_net, array_mappings
                )
                if array_candidate is None:
                    continue
                paired_pin, next_net = array_candidate
                if next_net not in visited_nets and (
                    next_net == endpoint_net
                    or not _is_reference_or_power_net(next_net)
                ):
                    candidates.append(
                        (
                            name,
                            ...
                            next_net,
                            "array_component",
                            f"map_pin {paired_pin} short_to {next_net}",
                        )
                    )
                continue
...
def trace_channel_paths(
    ...
    *,
    ...
    customer_array_catalog: list[dict[str, object]] | None = None,
    max_depth: int = 8,
) -> ChannelPathReport:
    ...
    if array_mapping_path is not None and customer_array_catalog is not None:
        raise ValueError(
            "legacy array_mapping_path and customer_array_catalog cannot be used together"
        )
    array_mappings = (
        array_mapping_library_from_customer_catalog(customer_array_catalog)
        if customer_array_catalog is not None
        else load_array_mapping_library(array_mapping_path)
    )
    _ensure_pyedb_path()
