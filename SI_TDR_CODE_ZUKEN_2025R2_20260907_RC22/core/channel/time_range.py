...
# 9.4.7.1: strict Config 생성 단계에서 route 유도 view로부터 해석 stop을
# 계산하는 임시 공식(고객 확정 대기).  resolver 계약과 무관한 공용 helper다.
ROUTE_VIEW_STOP_MULTIPLIER = 2.0
# 1500ps = 0.5ns 올림 취지와 내부 uniform step 7.5ps의 최소공배수.
# stop이 7.5ps 정수배가 아니면 AEDT uniform CSV exporter가 마지막 적응 구간을
# 채우지 못해 꼬리에 비유한값이 생긴다 (2026-08-20 R21 DP 배치, stop 8000ps 관측).
ROUTE_VIEW_STOP_QUANTIZATION_PS = 1500.0
ROUTE_VIEW_STOP_MINIMUM_PS = 6000.0
ROUTE_VIEW_DERIVED_STOP_SOURCE = (
    "derived-from-route-view x2 ceil1.5ns min6ns "
    "(provisional, customer approval pending)"
)


def derive_route_view_transient_stop_ps(view_max_ps: float) -> float:
    """stopPs = 2 x 유도된 viewMaxPs, 0.5ns 단위 올림, 최소 5000ps."""

    value = float(view_max_ps)
    if not math.isfinite(value) or value <= 0:
        raise TimeRangeResolutionError(
            f"route-derived view max must be a positive finite value: {view_max_ps!r}"
        )
    raw = ROUTE_VIEW_STOP_MULTIPLIER * value
    quantized = (
        math.ceil(raw / ROUTE_VIEW_STOP_QUANTIZATION_PS)
        * ROUTE_VIEW_STOP_QUANTIZATION_PS
    )
    if not math.isfinite(quantized):
        raise TimeRangeResolutionError(
            "route-derived transient stop produced a non-finite value"
        )
    return max(ROUTE_VIEW_STOP_MINIMUM_PS, quantized)
...
def _explicit_transient_stop(
    ...
    *,
    issues: list[dict[str, str]],
    internal_analysis_stop_ps: float | None = None,
    internal_analysis_stop_source: str | None = None,
) -> tuple[float, dict[str, Any]] | None:
    transient = tdr.get("transient") or {}
    if "stopPs" not in transient:
        if internal_analysis_stop_ps is not None:
            # strict 고객 경로: solve/data stop은 Run Config가 아니라 runtime
            # 내부 고정값이며, 호출자가 그 실측값과 출처를 명시적으로 넘긴다.
            # 숨은 default가 아니라 실제 해석 stop의 명시적 주입이다.
            value = float(internal_analysis_stop_ps)
            if not math.isfinite(value) or value <= 0:
                _add_issue(
                    issues,
                    "invalid_internal_analysis_stop",
                    "tdr.transient.stopPs",
                    "internal analysis stop must be a positive finite value",
                )
                return None
            return value, {
                "value": value,
                ...
                "canonicalUnit": "ps",
                "source": (
                    internal_analysis_stop_source
                    or "internal-runtime-transient-stop"
                ),
                "explicitConfigStop": False,
            }
        _add_issue(
            issues,
    ...
    if result is not None and transient.get("stopSource"):
        # 생성 단계가 기록한 stop 출처를 evidence로 관통시킨다.
        value, evidence = result
        evidence = {**evidence, "source": str(transient["stopSource"])}
...
def resolve_tdr_time_range(
    ...
    *,
    ...
    if observation_configured:
        ...
        analysis_stop_result = _explicit_transient_stop(
            tdr,
            issues=issues,
            internal_analysis_stop_ps=internal_analysis_stop_ps,
            internal_analysis_stop_source=internal_analysis_stop_source,
        )
    else:
    ...
    if (
        ...
        if observation_configured:
            ...
            calculation["analysisWindow"] = {
                ...
                "source": (
                    analysis_stop_result[1].get("source")
                    or "tdr.transient.stopPs"
                ),
