...
@dataclass(frozen=True)
class ExternalRequestPreparation:
    resolution: JobInputResolution
    generated_run_config: Path


@dataclass(frozen=True)
class ExternalCustomerBatchPreparation:
    """License-independent handoff from an external request to batch Run Configs."""

    request: ExternalRequestPreparation
    outcome: ConfigGenerationOutcome
...
def _generate(
    ...
    *,
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Internal staged Job root. Required when base_config contains "
            "analysisOptions; option files are resolved only from "
            "Setup/SWS, Setup/SFSDF, and Setup/TDR below it."
        ),
    )
    ...
    customer_array_catalog: list[dict[str, Any]] | None = None
    if strict_customer_mode:
        if args.reference_layer is not None or args.port_layer is not None:
            parser.error(
                "strict customer generation resolves both terminals from each endpoint Pin; "
                "fixed --reference-layer/--port-layer are forbidden"
            )
        if args.array_mapping is not None or args.series_treatment is not None:
            parser.error(
                "strict customer generation cannot use legacy --array-mapping or "
                "--series-treatment"
            )
        if "seriesTreatment" in base_payload or "seriesModels" in base_payload:
            parser.error(
                "strict customer generation does not accept seriesTreatment or seriesModels"
            )
        assert customer_component_handling is not None
        customer_array_catalog = customer_component_handling.get("arrayCatalog")
        if not isinstance(customer_array_catalog, list):
            parser.error(
                "strict customer generation requires customerComponentHandling.arrayCatalog"
            )

    analysis_option_catalog = None
    if "analysisOptions" in base_payload:
        if args.job_root is None:
            parser.error("base config analysisOptions requires --job-root")
        try:
            analysis_option_catalog = resolve_analysis_option_catalog(
                base_payload,
                job_root=args.job_root,
            )
        except (AnalysisOptionError, OSError) as exc:
            parser.error(str(exc))
    ...
    if not strict_customer_mode and has_inline_bom_rules:
    ...
    if args.minimal_profile:
        ...
        discovery_result = write_channel_path_report(
            ...
            customer_array_catalog=customer_array_catalog,
        ...
        if args.snp_file:
            ...
            selected_detailed_batch = selected_batch
            ...
            if len(normalization_result.batches) == 1:
                selected_detailed_batch = normalization_result.batches[0]
                selected_snp_file = selected_detailed_batch.snp_file
        write_detailed_channel_targets_csv(
            normalization_result,
            normalized_csv_path,
    ...
    if analysis_option_catalog is not None:
        batch_id = selected_snp_file or prefix
        if selected_detailed_batch is not None:
            syz_profile_id = selected_detailed_batch.syz_option
            tdr_profiles_by_item = selected_detailed_batch.tdr_options_by_item()
        else:
            syz_profile_id = None
            tdr_profiles_by_item = {
                target.report_group or target.channel: None for target in targets
            }
        try:
            analysis_option_selection = select_analysis_options(
                analysis_option_catalog,
                batch_id=batch_id,
                syz_profile_id=syz_profile_id,
                tdr_profile_ids_by_item=tdr_profiles_by_item,
            )
        except AnalysisOptionError as exc:
            parser.error(str(exc))
    ...
    config = build_run_config(
        ...
        analysis_settings={
            ...
            **(
                {}
                if strict_customer_mode
                else {"referenceLayer": args.reference_layer}
            ),
        },
        input_provenance=input_provenance,
        analysis_option_selection=analysis_option_selection,
        customer_component_handling=customer_component_handling,
    )
    ...
    resolved_syz_profile_id = (
        analysis_option_selection["syz"]["profileId"]
        if analysis_option_selection is not None
        else None
    )
    resolved_tdr_profile_ids = (
        {
            item_id: selection["profileId"]
            for item_id, selection in analysis_option_selection["tdr"].items()
        }
        if analysis_option_selection is not None
        else {}
    )
    ...
    manifest = {
        ...
        "customerComponentHandling": (
            customer_component_handling if strict_customer_mode else None
        ),
        ...
        "syzProfileId": resolved_syz_profile_id,
        "tdrProfileIdsByItem": resolved_tdr_profile_ids,
        "analysisOptionSelection": analysis_option_selection,
        "summary": {
            "targets": len(targets),
            ...
            "componentHandlingMode": (
                "customer-strict" if strict_customer_mode else "legacy"
            ),
...
def prepare_external_request(
    ...
    *,
    ...
    generated_run_config_path: Path,
) -> ExternalRequestPreparation:
    """Resolve a Heaven request and persist its combined runtime snapshot."""

    resolution = resolve_job_inputs(request_path)
    administrator_config = _read_json_object(
        administrator_config_path,
        label="administrator config",
    )
    generated = build_generated_run_config(
        administrator_config,
        resolution,
        administrator_config_path=administrator_config_path,
    )
    output_path = generated_run_config_path.resolve()
    _write_json(output_path, generated)
    return ExternalRequestPreparation(
        resolution=resolution,
        generated_run_config=output_path,
    )


def _validated_external_reference_result(
    reference_result: ReferencePreprocessResult,
    *,
    generated_config: dict[str, Any],
) -> None:
    """Validate the mocked/live reference handoff before any Channel Path work."""

    try:
        loaded = load_reference_preprocess_manifest(reference_result.manifest_path)
    except ReferencePreprocessError as exc:
        raise ConfigGenerationError(
            f"external reference preprocess manifest is invalid: {exc}"
        ) from exc
    if loaded.manifest_id != reference_result.manifest_id:
        raise ConfigGenerationError(
            "reference preprocess result manifestId does not match its manifest"
        )
    if loaded.reference_siw != reference_result.reference_siw.resolve():
        raise ConfigGenerationError(
            "reference preprocess result SIW does not match its manifest"
        )
    if loaded.reference_aedb != reference_result.reference_aedb.resolve():
        raise ConfigGenerationError(
            "reference preprocess result AEDB does not match its manifest"
        )
    if loaded.aedt_version != reference_result.aedt_version:
        raise ConfigGenerationError(
            "reference preprocess result AEDT version does not match its manifest"
        )
    if loaded.design_input_type != reference_result.design_input_type:
        raise ConfigGenerationError(
            "reference preprocess result designInputType does not match its manifest"
        )

    expected_version = str(generated_config.get("aedtVersion") or "").strip()
    actual_version = str(reference_result.aedt_version or "").strip()
    if not expected_version or actual_version != expected_version:
        raise ConfigGenerationError(
            "reference preprocess AEDT version differs from administrator Config: "
            f"reference={actual_version!r}, administrator={expected_version!r}"
        )
    expected_design_type = str(generated_config.get("designInputType") or "").strip()
    if reference_result.design_input_type != expected_design_type:
        raise ConfigGenerationError(
            "reference preprocess designInputType differs from administrator Config: "
            f"reference={reference_result.design_input_type!r}, "
            f"administrator={expected_design_type!r}"
        )


def prepare_external_customer_batches(
    request_path: Path,
    *,
    administrator_config_path: Path,
    reference_result: ReferencePreprocessResult,
    work_root: Path,
    generated_run_config_path: Path | None = None,
    progress: ConsoleProgress | None = None,
) -> ExternalCustomerBatchPreparation:
    """Generate strict per-sNp Run Configs from the immutable Heaven request.

    This is the FB-06 production seam consumed by the later one-command
    orchestrator.  It deliberately starts after reference preprocessing so the
    Channel Path/component/Port contract can be tested without an Ansys license.
    """

    work_root = work_root.resolve()
    snapshot_path = (
        generated_run_config_path.resolve()
        if generated_run_config_path is not None
        else work_root / "request" / "generated_run_config.json"
    )
    external = prepare_external_request(
        request_path,
        administrator_config_path=administrator_config_path,
        generated_run_config_path=snapshot_path,
    )
    generated = _read_json_object(
        external.generated_run_config,
        label="generated external Run Config snapshot",
    )
    _validated_external_reference_result(
        reference_result,
        generated_config=generated,
    )

    raw_customer_component_handling = generated.get("customerComponentHandling")
    if not isinstance(raw_customer_component_handling, dict):
        raise ConfigGenerationError(
            "generated external Run Config requires customerComponentHandling"
        )
    customer_component_handling = dict(raw_customer_component_handling)
    customer_component_handling["jobRoot"] = str(
        external.resolution.job_root.resolve()
    )
    if customer_component_handling.get("series") != "short":
        raise ConfigGenerationError(
            "generated external Run Config must select the strict Series short dispatcher"
        )
    if customer_component_handling.get("array") != "bom-configured-source-column-authoritative":
        raise ConfigGenerationError(
            "generated external Run Config must select the configured BOM source "
            "column authoritative Array policy"
        )

    spec_path = external.resolution.input("Spec").path
    try:
        detailed = normalize_detailed_csv(spec_path)
    except DetailedInputValidationError as exc:
        raise ConfigGenerationError(str(exc)) from exc
    if detailed.unresolved:
        raise ConfigGenerationError(
            "external customer Spec contains unresolved rows; Channel Path generation "
            "is blocked before solver use"
        )
    if not detailed.batches:
        raise ConfigGenerationError("external customer Spec produced no sNp batches")

    ports = generated.get("ports") or {}
    if not isinstance(ports, dict):
        raise ConfigGenerationError("generated external Run Config ports must be an object")
    generated["ports"] = ports
    if "referenceLayer" in ports:
        raise ConfigGenerationError(
            "strict external customer generation forbids ports.referenceLayer"
        )
    # 관리자 Config는 고정 정책을 선언하지 않는다. 여기서 채우되, 명시적으로
    # 다른 값이 들어와 있으면 그대로 실패시킨다.
    ports.setdefault("terminalLayerPolicy", "endpoint-pin-same-layer")
    ports.setdefault("portContractValidation", "strict")
    ports.setdefault("referenceNetPolicy", "edb-ground-auto-fail-closed")
    if ports.get("terminalLayerPolicy") != "endpoint-pin-same-layer":
        raise ConfigGenerationError(
            "strict external customer generation requires endpoint-pin-same-layer"
        )
    raw_impedance = ports.get("singleEndedImpedanceOhm", 50.0)
    if isinstance(raw_impedance, bool):
        raise ConfigGenerationError("ports.singleEndedImpedanceOhm must be positive")
    try:
        port_impedance = float(raw_impedance)
    except (TypeError, ValueError) as exc:
        raise ConfigGenerationError(
            "ports.singleEndedImpedanceOhm must be positive"
        ) from exc
    if port_impedance <= 0:
        raise ConfigGenerationError("ports.singleEndedImpedanceOhm must be positive")

    batch_root = work_root / "analysis_batches"
    batch_outcomes: list[tuple[str, ConfigGenerationOutcome]] = []
    for index, batch in enumerate(detailed.batches, start=1):
        output_dir = batch_root / batch.snp_file / "generated_config"
        argv = [
            str(spec_path),
            str(reference_result.reference_aedb),
            str(external.generated_run_config),
            "--job-root",
            str(external.resolution.job_root),
            "--output-dir",
            str(output_dir),
            "--name",
            batch.snp_file,
            "--reference-siw",
            str(reference_result.reference_siw),
            "--reference-preprocess-manifest",
            str(reference_result.manifest_path),
            "--aedt-version",
            str(reference_result.aedt_version),
            "--port-impedance-ohm",
            str(port_impedance),
            "--strategy",
            "external-customer-metadata-snp",
            "--scope",
            "external-customer-channel-path",
            "--detailed-input",
            "--snp-file",
            batch.snp_file,
        ]
        if progress is not None:
            with progress.stage(
                "CONFIG-BATCH",
                "Generate strict customer analysis-batch Run Config",
                detail=(
                    f"{index}/{len(detailed.batches)} {batch.snp_file} "
                ...
                if outcome.status == 0:
                    stage.complete("customer-strict, unresolved=0")
                else:
                    stage.fail(
    ...
    aggregate_dir = batch_root / "generated_config"
    aggregate_dir.mkdir(parents=True, exist_ok=True)
    aggregate_manifest = aggregate_dir / "external_customer_batch_manifest.json"
    aggregate_unresolved = aggregate_dir / "external_customer_batch_unresolved.json"
    status = max(outcome.status for _, outcome in batch_outcomes)
    records: list[dict[str, Any]] = []
    for snp_file, outcome in batch_outcomes:
        child_manifest = (
            _read_json_object(
                outcome.manifest,
                label=f"strict customer batch {snp_file} manifest",
            )
            if outcome.manifest.is_file()
            else {}
        )
        records.append(
            {
                ...
                "channelPathReport": child_manifest.get("channelPathReport"),
                "analysisOptionSelection": child_manifest.get(
                    "analysisOptionSelection"
                ),
                "componentHandlingMode": "customer-strict",
            }
        )
    _write_json(
        ...
        {
            "version": 1,
            "status": "ok" if status == 0 else "blocked_batch_generation",
            "batches": [record for record in records if record["status"] != 0],
        },
    )
    _write_json(
        aggregate_manifest,
        {
            "version": 1,
            "status": "ok" if status == 0 else "blocked_batch_generation",
            "inputFormat": "external-heaven-detailed-spec",
            "externalRequest": str(external.resolution.request_path),
            "generatedRunConfig": str(external.generated_run_config),
            "referencePreprocessManifest": str(reference_result.manifest_path),
            "referencePreprocessManifestId": reference_result.manifest_id,
            "aedtVersion": reference_result.aedt_version,
            "snpFileCount": len(records),
            "runConfigs": [record["runConfig"] for record in records],
            "combinedUnresolved": str(aggregate_unresolved),
            "batches": records,
        },
    )
    ...
    return ExternalCustomerBatchPreparation(
        request=external,
        outcome=ConfigGenerationOutcome(
            status=status,
            ...
            batch_manifest=aggregate_manifest,
        ),
    )
...
def prepare_from_request(
    ...
    *,
    ...
    elif preprocess_result is not None and preprocess_result.aedt_version is not None:
        aedt_version = preprocess_result.aedt_version
    else:
        aedt_version = "2024.2"
    if (
    ...
    for snp_file, outcome in batch_outcomes:
        child_manifest = (
            _read_json_object(
                outcome.manifest,
                label=f"analysis batch {snp_file} manifest",
            )
            if outcome.manifest.is_file()
            else {}
        )
