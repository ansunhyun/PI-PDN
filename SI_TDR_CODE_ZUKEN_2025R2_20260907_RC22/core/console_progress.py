...
class ConsoleProgress:
    ...
    def _stage_prefix(self, area: str, state: str, *, depth: int = 1) -> str:
        normalized_depth = max(1, depth)
        marker = ">" * normalized_depth
        return (
            f"{self._tree_prefix(normalized_depth, marker)} "
            f"[SI-TDR][{area}][{state}]"
        )
    ...
    def stage(
        ...
        *,
        ...
        normalized_depth = max(1, depth)
        self._emit(
            f"{self._stage_prefix(area, 'START', depth=normalized_depth)} {label}"
        )
        if detail:
            self.detail("Details", detail, depth=normalized_depth + 1)
        stage = ProgressStage()
        try:
            yield stage
            ...
            self._emit(
                f"{self._stage_prefix(area, 'FAIL', depth=normalized_depth)} "
                f"{label} failed "
                f"({elapsed})"
            )
            self.detail("Cause", reason, depth=normalized_depth + 1)
            raise
        else:
            elapsed = format_elapsed(self._clock() - started)
            if stage.failure:
                ...
                self.detail(
                    "Cause",
                    stage.failure,
                    depth=normalized_depth + 1,
                )
            else:
                suffix = f" - {stage.result}" if stage.result else ""
                self._emit(
                    f"{self._stage_prefix(area, 'DONE', depth=normalized_depth)} "
                    f"{label} completed "
                    f"({elapsed}){suffix}"
                )
    ...
    def event(
        ...
        *,
        ...
        self._emit(f"{self._stage_prefix(area, state, depth=depth)} {message}")
    ...
    def item(
        ...
        *,
        ...
        normalized_depth = max(1, depth)
        marker = ">" * normalized_depth
        prefix = self._tree_prefix(normalized_depth, marker)
        self._emit(
            f"{prefix} [SI-TDR][{area}][{index}/{total}][{state}] {message}"
        )

    def info(self, area: str, message: str, *, depth: int = 1) -> None:
        self.event(area, "INFO", message, depth=depth)

    def warning(self, area: str, message: str, *, depth: int = 1) -> None:
        self.event(area, "WARN", message, depth=depth)

    def error(self, area: str, message: str, *, depth: int = 1) -> None:
        self.event(area, "ERROR", message, depth=depth)
