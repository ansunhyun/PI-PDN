...
class AnsysReferenceBackend:
    ...
    def acquire_zuken(
        ...
        source_dir = staging_dir / "zuken_source"
        ...
        exports = request.zuken_exports or {
            "exportANF": True,
            "exportCMP": True,
            "exportODB": False,
            "exportEDB": False,
        }
        if not exports.get("exportANF") or not exports.get("exportCMP"):
            raise ReferencePreprocessError(
                "zuken_design common reference build requires exportANF and exportCMP"
            )
        ...
        optional_exports: list[dict[str, Any]] = []
        if exports.get("exportODB"):
            export_odb = request.zuken_bin / "DFodbout.exe"
            if not export_odb.is_file():
                raise ReferencePreprocessError(f"Zuken executable not found: {export_odb}")
            argv = [str(export_odb), "-r", str(dsgn), "-o", str(dsgn.parent)]
            result = self._run_command(argv)
            optional_exports.append(self._command_record(argv, result, 1))
            if result.returncode != 0:
                raise ReferencePreprocessError(
                    f"Zuken DSGN-to-ODB++ conversion failed: returncode={result.returncode}"
                )
        if exports.get("exportEDB"):
            export_edb = request.zuken_bin / "DFaedbout.exe"
            if not export_edb.is_file():
                raise ReferencePreprocessError(f"Zuken executable not found: {export_edb}")
            edb_path = dsgn.with_suffix(".aedb")
            argv = [str(export_edb), "-r", str(dsgn), "-o", str(edb_path)]
            result = self._run_command(argv)
            optional_exports.append(self._command_record(argv, result, 1))
            if result.returncode != 0:
                raise ReferencePreprocessError(
                    f"Zuken DSGN-to-EDB conversion failed: returncode={result.returncode}"
                )
        return AcquiredAnfCmp(
            ...
            evidence={
                ...
                "configuredExports": dict(exports),
                "optionalExportCommands": optional_exports,
            },
        )
    ...
    def build_from_anf_cmp(
        ...
        base_siw = staging_dir / f"{request.output_name}__import.siw"
        base_aedb = staging_dir / f"{request.output_name}__import.aedb"
        ...
        working_aedb = staging_dir / f"{request.output_name}__working.aedb"
        ...
        reference_siw = reference_dir / f"{request.output_name}_ref.siw"
        reference_aedb = reference_dir / f"{request.output_name}_ref.aedb"
    ...
    def _apply_dcir_reference_policy(
        ...
        preserve_types = set(dc_short.get("preserveCompTypes", []))
        ...
        bom_missing_delete_components: set[str] = set()
        short_component_deletes: set[str] = set()
        ...
        try:
            ...
            for name, component in components.items():
                ...
                if component_type in preserve_types:
                    ...
                    bom_missing_delete_components.add(name)
                else:
            ...
            for name, component in components.items():
                ...
                short_component_deletes.add(name)

            # Components omitted from the assembly BOM/PartList are DNP parts.
            # Remove those circuit elements while the EDB is still open instead
            # of deferring them to ScrDeleteCktElem after SIWave import.  The
            # latter rejects valid ANF/CMP elements such as JP1009 in 2025 R2.
            # A configured DC-short component remains in the SIWave operation
            # set because its connected nets must be merged before deletion.
            bom_only_deletes = sorted(
                bom_missing_delete_components - short_component_deletes
            )
            edb_deleted: list[str] = []
            for name in bom_only_deletes:
                try:
                    result = edb._components.delete(name)
                except Exception as exc:
                    raise ReferencePreprocessError(
                        f"failed to delete BOM-missing component in EDB: {name}: {exc}"
                    ) from exc
                if result is not True:
                    raise ReferencePreprocessError(
                        f"failed to delete BOM-missing component in EDB: {name}"
                    )
                edb_deleted.append(name)
            edb.save()
        finally:
            try:
                edb.close_edb()
        ...
        return {
            ...
            "bomMissingDeleteCandidates": sorted(bom_missing_delete_components),
            "bomMissingDeletedInEdb": edb_deleted,
            "deleteComponents": sorted(short_component_deletes),
            "shortCorrections": {
    ...
    def export_aedb_from_siw(
        ...
        Siwave = self._siwave_class()
        reference_siw = reference_dir / request.reference_siw.name
        reference_aedb = reference_dir / f"{request.output_name}_ref.aedb"
