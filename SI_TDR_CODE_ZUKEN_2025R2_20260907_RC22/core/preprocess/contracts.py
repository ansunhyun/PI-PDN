...
PREPROCESSOR_IMPLEMENTATION_VERSION = 6
...
class ReferencePreprocessRequest:
    ...
    design_input_type: str | None = None
    syz_profile_id: str | None = None
    source_cad: Path | None = None
    zuken_exports: dict[str, bool] | None = None
...
class ReferencePreprocessResult:
    ...
    def as_input_provenance(self) -> dict[str, Any]:
        ...
        if self.design_input_type is not None:
            provenance.update(
                {
                    "designInputType": self.design_input_type,
                    "preprocessingBranch": self.mode,
                    "designInputTypeSource": self.mode_source,
                    "syzProfileId": self.syz_profile_id,
                }
            )
        else:
            # Internal migration fixtures retain their explicit legacy vocabulary.
            # In particular, reference_siw never becomes a customer designInputType.
            provenance.update(
                {
...
def _validate_dcir_reference_policy(
    ...
    *,
    ...
    preserve_types = dc_short.get("preserveCompTypes", [])
    if not isinstance(preserve_types, list) or not all(
        isinstance(item, str) and item.strip() for item in preserve_types
    ):
        raise ReferencePreprocessError(
            "preprocessing.dcShort.preserveCompTypes must be a string array when supplied"
        )
    overlapping_types = set(dc_short["deleteCompTypes"]) & set(preserve_types)
