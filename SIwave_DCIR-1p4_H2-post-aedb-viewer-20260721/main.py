...
INPUT_DIR = Path()
db = None  
bom_info = {} # 💡 [추가] 전역에서 BOM 정보를 참조할 수 있도록 초기화
# endregion
...
def wait_for_edb_ready(edb_path: Path, timeout: float = 300.0, check_interval: float = 3.0, stable_checks: int = 2) -> bool:
    """Wait until EDB folder and non-empty edb.def are stably available."""
...
def ensure_pre_edb_saved(app, source_edb_path: Path, pre_edb_path: Path, max_retries: int = 2, timeout: float = 300.0):
    """
    Save PRE EDB robustly with retries.
    On retry, reopen source EDB to avoid stale/half-closed EDB handles.
    """
    last_error = None
...
# 💡 [수정] 파라미터에 bulk_inductor_list 추가
def run_dcir_case(
    case,
    idx,
    total_cases,
    ...
    try:
        ...
        # 💡 [수정] SIwave.py로 bulk_inductor_list 전달
        ...
        if run_solve:
            ...
            if result.returncode:
                # 정상 해석 시에는 로그를 남기지 않음 (solver 진행 출력이 케이스마다 수백 줄이라 로그가 커짐)
...
try:
    ...
    if INPUT_CAD_FILE.suffix == '.zip':
        ...
        if conf_manager.data['DCIR']['isZuken']:
            ...
            if conf_manager.data['DCIR']['exportANF']:
            ...
            if conf_manager.data['DCIR']['exportODB']:
            ...
            if conf_manager.data['DCIR']['exportEDB']:
                DSGN2EDB_EXEC = ZUKEN_BIN_DIR / 'DFaedbout.exe'
                EDB_PATH = DSGN_FILE.with_suffix('.aedb')
                result = subprocess.run([str(DSGN2EDB_EXEC), '-r', str(DSGN_FILE), '-o', str(EDB_PATH)], capture_output=True, text=True)
    ...
    logger.log(f"Create SIwave Project and Export EDB", level=LogLevel.DETAIL1)
    
    app = SIwave(version=AEDT_VERSION, logger=logger)
    STK_FILE = input_valchk._default_inputFiles['Stackup']
    base_name = ANF_FILE.stem.split('-')[0]
    SIwave_FILE_PATH = INPUT_DIR / f'{base_name}.siw'
    EDB_FILE_PATH = INPUT_DIR / f'{base_name}.aedb'
    
    app.create_project(ANF_FILE, CMP_FILE, STK_FILE, SIwave_FILE_PATH, EDB_FILE_PATH)
    ...
    if 'app' in locals(): app.quit_application()
    step += 1
# endregion
...
try:
    ...
    bom_info = settings_manager.get_bom() # 💡 [참고] 여기서 파싱된 bom_info가 전역 변수에 저장됨
    ...
    for idx, case in enumerate(spec_info):
        ...
        target_pin_name = str(case['IC_Pin_No.']).strip()
        ...
        if not pin_inst:
            ...
            target_net_from_spec = case.get('Net_Name') or case.get('Target_Net')
        ...
        dcir_cases_info.append({
            ...
            'Vmag': case['Voltage_(V)'], 'Imag': case['Current_(A)'],
            'MinSpec': case['Min_Spec_(V)'], 'MaxSpec': case['Max_Spec_(V)']
        })
...
try:
    ...
    for idx, case in enumerate(dcir_cases_info):
        case_record = run_dcir_case(
            ...
            exec_file=exec_file,
            bulk_inductor_list=bom_info.get('bulkInd', []), # 💡 [수정] BOM의 Power Inductor 리스트 전달
            run_solve=(STAGE != "pre")
        )
        if case_record:
