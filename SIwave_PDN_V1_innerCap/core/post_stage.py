...
def export_post_edb(
    ...
    A previous Post AEDB is removed before export so a failed rerun can never
    silently fall back to stale geometry or source settings. Partial exports are
    also removed and the failure is propagated to the caller.
    """
...
def find_run_candidates(project: Path) -> list[RunArtifacts]:
    ...
    for folder in result_root.iterdir():
        ...
        runs.append(RunArtifacts(
            ...
            ced=_resolve_run_artifact(folder, run_name, "ced"),
...
def find_completed_runs(project: Path) -> list[RunArtifacts]:
    return [
        ...
        if run.siw.is_file() and run.ced.is_file() and run.finished.is_file()
    ]
...
def read_source_settings(siw_path: Path, v_port_name: str, i_port_name: str) -> dict[str, str]:
    expected = {"VSRC": v_port_name, "CSRC": i_port_name}
    values: dict[str, str] = {}

    with siw_path.open("r", encoding="utf-8", errors="replace") as stream:
        for raw_line in stream:
            ...
            if len(fields) > 10 and fields[1] == expected[kind]:
                values[kind] = fields[10]
                if len(values) == len(expected):
                    break

    missing = [expected[kind] for kind in expected if kind not in values]
    if missing:
        raise PostStageError(f"Expected source(s) not found in {siw_path}: {', '.join(missing)}")
    return {"Vmag": values["VSRC"], "Imag": values["CSRC"]}


def read_ced_result(ced_path: Path, v_port_name: str, i_port_name: str) -> dict[str, Any]:
    source_rows: dict[str, list[str]] = {}
    with ced_path.open("r", encoding="utf-8", errors="replace") as stream:
        for raw_line in stream:
            fields = shlex.split(raw_line.strip(), posix=True)
            if fields and fields[0] in (v_port_name, i_port_name):
                source_rows[fields[0]] = fields

    missing = [name for name in (v_port_name, i_port_name) if name not in source_rows]
    if missing:
        raise PostStageError(f"Expected result source(s) not found in {ced_path}: {', '.join(missing)}")

    i_row = source_rows[i_port_name]
    v_row = source_rows[v_port_name]
    if len(i_row) < 7 or len(v_row) < 7:
        raise PostStageError(f"Unexpected CED source row format: {ced_path}")

    try:
        return {
            "load_voltage": float(i_row[2]),
            "result_current": float(i_row[5]),
            "i_source_net": i_row[1],
            "v_source_net": v_row[1],
            "gnd_net": i_row[3],
        }
    except ValueError as exc:
        raise PostStageError(f"Invalid numeric result in {ced_path}: {exc}") from exc


def _calculate_metrics(v_mag: str, load_voltage: float, min_spec: str, max_spec: str) -> dict[str, Any]:
    source_voltage = float(v_mag)
    drop_voltage = round(source_voltage - load_voltage, 3)
    drop_rate = 0.0 if abs(source_voltage) < 1e-12 else drop_voltage / source_voltage * 100
    passed = float(min_spec) < load_voltage < float(max_spec)
    return {
        "Result": round(load_voltage, 3),
        "Drop Voltage": drop_voltage,
...
def _base_summary(record: dict[str, Any], edb_path: Path) -> dict[str, Any]:
    return {
        ...
        "DCDC_name": record.get("Source_Component", ""),
        "DCDC_pin": record.get("Source_Pin", ""),
        "DCDC_net": record.get("Net_Chain", []),
        "Full_Net_Chain": record.get("Full_Net_Chain", []),
        "Vmag": str(record.get("Voltage_V", "0")),
        "Imag": str(record.get("Current_A", "0")),
        "MinSpec": str(record.get("Min_Spec_V", "0")),
        "MaxSpec": str(record.get("Max_Spec_V", "0")),
        "is_done": False,
        "edb": edb_path,
        "_viewer_siw": None,
        "Result": 0.0,
...
def _run_history_entry(run: RunArtifacts, record: dict[str, Any]) -> tuple[dict[str, Any], str]:
    settings = read_source_settings(
        run.siw,
        str(record.get("V_Port_Name", "")),
        str(record.get("I_Port_Name", "")),
    )
    result = read_ced_result(
        run.ced,
        str(record.get("V_Port_Name", "")),
        str(record.get("I_Port_Name", "")),
    )
    target_net = str(record.get("Target_Net", ""))
    if result["i_source_net"] != target_net:
        raise PostStageError(
            f"Current source net changed for case {record.get('Case_Index')}: "
            f"expected {target_net}, got {result['i_source_net']}"
        )
    metrics = _calculate_metrics(
        settings["Vmag"],
        result["load_voltage"],
        str(record.get("Min_Spec_V", "0")),
        str(record.get("Max_Spec_V", "0")),
    )
    entry = {
        "Run": run.name,
        ...
        "Drop Voltage": metrics["Drop Voltage"],
        "Drop Rate": metrics["Drop Rate"],
        "Pass/Fail": metrics["Pass/Fail"],
        "I_Source_Net": result["i_source_net"],
        "V_Source_Net": result["v_source_net"],
        "Result_Current": result["result_current"],
    }
    return entry, str(result["gnd_net"])
...
def _build_case(record: dict[str, Any], output_dir: Path) -> tuple[dict[str, Any], dict[str, Any], str | None, list[float]]:
    ...
    history = {
        ...
        "Baseline": {"Vmag": summary["Vmag"], "Imag": summary["Imag"]},
        "Latest_Run": None,
        "Changed": False,
        "Changes": [],
        "Runs": [],
    ...
    try:
        ...
        latest_run = runs[-1]
        summary["Vmag"] = latest["Vmag"]
        summary["Imag"] = latest["Imag"]
        summary["_viewer_siw"] = latest_run.siw
        summary.update({
            "is_done": True,
            "Result": latest["Result"],
            "Drop Voltage": latest["Drop Voltage"],
            "Drop Rate": latest["Drop Rate"],
            "Pass/Fail": latest["Pass/Fail"],
        })
        ...
        summary["Field_Case"] = output_dir / f"Field_{display_name}.case"
        summary["Mesh_Case"] = output_dir / f"Mesh_{display_name}.case"

        changes = []
        for field, baseline_key in (("Vmag", "Vmag"), ("Imag", "Imag")):
            before = history["Baseline"][baseline_key]
            after = latest[field]
            if not math.isclose(float(before), float(after), rel_tol=0.0, abs_tol=1e-12):
...
def _merge_legacy_metadata(record: dict[str, Any], existing: dict[tuple[str, str], dict[str, Any]]) -> dict[str, Any]:
    ...
    if not merged.get("Source_Pin"):
        merged["Source_Pin"] = previous.get("DCDC_pin", "")
    return merged
