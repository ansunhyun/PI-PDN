...
echo ============================================================
echo PDN Auto Sim - Stage FULL Runner
echo Root : %ROOT_DIR%
echo App  : %APP_DIR%
...
if /I "%PDN_USE_PRECONVERTED%"=="1" (
    ...
    if not defined HAS_PRECONV_AEDB (
        ...
        echo [INFO] If .dsgn exists, flow will try DSGN->EDB export automatically.
    )
)
echo Effective Preconverted Mode : %PDN_USE_PRECONVERTED%

if not exist "%APP_DIR%\main.py" (
    echo [ERROR] main.py not found: %APP_DIR%\main.py
    exit /b 1
)
...
if /I "%PDN_USE_PRECONVERTED%"=="1" (
    echo [INFO] Preconverted mode enabled. Zuken converter existence check is skipped.
) else (
...
echo.
echo [RUN] python main.py "!RUN_JSON!" --stage full
cd /d "%APP_DIR%"
...
python main.py "!RUN_JSON!" --stage full
set "EXIT_CODE=%ERRORLEVEL%"

echo.
if "%EXIT_CODE%"=="0" (
    echo [DONE] Stage full completed successfully.
) else (
    echo [FAIL] Stage full failed with exit code %EXIT_CODE%.
)
